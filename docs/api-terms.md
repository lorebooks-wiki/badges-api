...
- You follow and abide by the [Community Code of Conduct] and
  [lorebooks.wiki ToS].
- You do not spam API requests to `/badges/<project>/<badge-name>` endpoint(s)
...
For API authenication to be implemented soon, we use GitHub as our authenication
service by passing your personal access token as an `Authorization` bearer
token. We do not store raw tokens in our KV store, instead they are one-way
hashed to SHA512 for purposes of caching GitHub API requests and to check user
permissions with less latency.
