...
program
  ...
  .action(async (iconName: string, base64Data: string) => {
    ...
    try {
      const data = await kvApi.set(["badgeIcons", iconName], base64Data);
      logger.debug(`${data.versionstamp}`);
      logger.success(`Upload succeeded to KV`);
    } catch (error) {
...
program
  ...
  .action(async (project, name, options) => {
    ...
    if (options.type == "badge") {
      ...
      if (data.logo != null) {
        const badgeIconData = (await kvApi.get(["badgeIcons", data.logo]))
          .value;
