...
program
  .command("admins:add")
  .description(
    "grant an GitHub user admin permissions by adding to @lorebooks-wiki/api-admins team",
  )
  .argument("<username>", "GitHub username")
  .alias("add-admin")
  .action(async (username) => {
    try {
      const { data, headers } = await ghApi.teams
      ...
      logger.success(
        `Successfully added @${username} to @lorebooks-wiki/api-admins team.`,
      );
  ...
  .action(async (username, options) => {
    try {
      ...
      if (options.removeFromOrg == true) {
        ...
        logger.success(
          `Successfully removed @${username} from @lorebooks-wiki/api-admin team and lorebooks-wiki org`,
        );
      } else {
        logger.success(
          `Successfully removed @${username} from @lorebooks-wiki/api-admins team.`,
        );
      }
    } catch (err) {
